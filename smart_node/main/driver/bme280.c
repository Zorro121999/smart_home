...
static void transduce_temp(uint8_t *byte_value, uint8_t *calib_value, float *value);
static void transduce_humidity(uint8_t *byte_value, uint8_t *calib_value1, uint8_t *calib_value2, float *value);
...
static void compensate_temperature(uint32_t *raw_value, uint8_t *calib_value, float *value);
static void compensate_humidity(uint32_t *raw_value, uint8_t *calib_value1, uint8_t *calib_value2, float *value);
...
void bme280_measure_temp(spi_device_handle_t spi, uint8_t *temp_cal, float *value) {
    ...
    temp[1] = reg_r[4];
    temp[2] = reg_r[5];
    transduce_temp(temp, temp_cal, value);
...
void bme280_measure_humidity(spi_device_handle_t spi, uint8_t *hum_cal1, uint8_t *hum_cal2, float *value) {
    ...
    // config_reg[0] = ENABLE_HUM_REG;
    // config_reg[1] = 0x02;
    // memset(&t, 0, sizeof(t));
    // t.length = 16;
    // t.tx_buffer = config_reg;
    // ret = spi_device_polling_transmit(spi, &t);
    ...
    hum[0] = reg_r[6];
    hum[1] = reg_r[7];
    transduce_humidity(hum, hum_cal1, hum_cal2, value);

}

static void transduce_temp(uint8_t *byte_value, uint8_t *calib_value, float *value) {
    uint32_t raw_value = 0;
    raw_value = ((uint32_t)byte_value[1] << 12) |
    ((uint32_t)byte_value[2] << 4)  |
    ((uint32_t)byte_value[3] >> 4);
    compensate_temperature(&raw_value, calib_value, value);
}

static void transduce_humidity(uint8_t *byte_value, uint8_t *calib_value1, uint8_t *calib_value2, float *value) {
    uint32_t raw_value = 0;
    raw_value = ((uint32_t)byte_value[1] << 8) |
    ((uint32_t)byte_value[2] << 0);
    compensate_humidity(&raw_value, calib_value1, calib_value2, value);
}
...
static void get_humidity_calib(spi_device_handle_t spi, uint8_t *calib_value1, uint8_t *calib_value2) {
    ...
    ret = spi_device_polling_transmit(spi, &t);
    memset(&t, 0, sizeof(t));
    uint8_t reg_t2[9] = {HUM_COMP_REG2, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    t.length = 72;
    t.tx_buffer = reg_t2;
...
static void compensate_humidity(uint32_t *raw_value, uint8_t *calib_value1, uint8_t *calib_value2, float *value) {
    ...
    int16_t dig_h2 = ((int16_t)calib_value2[2]) | ((int16_t)calib_value2[3] << 8);
    uint8_t dig_h3 = calib_value2[4];
    int16_t dig_h4 = ((int16_t)calib_value2[6] | 0x0001) | ((int16_t)calib_value2[5] << 4);
    int16_t dig_h5 = ((int16_t)calib_value2[6] >> 4) | ((int16_t)calib_value2[7] << 8);
    int32_t t_fine = 0;
    uint8_t dig_h6 = calib_value2[8];

    var1 = t_fine - ((int32_t)76800);
    var2 = (int32_t)(*raw_value * 16384);
    ...
    *value = (float)(humidity/humidity_max) * 100.0f;
}